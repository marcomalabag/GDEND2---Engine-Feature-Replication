...
struct Matrix4 final : public DirectX::XMFLOAT4X4
{
	explicit Matrix4();

	explicit Matrix4(const Vector3D& r0,
	                 const Vector3D& r1,
	                 const Vector3D& r2);
	...
	static Matrix4 CreateTranslation(const Vector3D& position) noexcept;
	static Matrix4 CreateTranslation(float x,
	                                 float y,
	                                 float z) noexcept;

	static Matrix4 CreateScale(const Vector3D& scales) noexcept;
	...
	static Matrix4 CreatePerspectiveFieldOfView(float fov,
	                                            float aspectRatio,
	                                            float nearPlane,
	                                            float farPlane) noexcept;
	static Matrix4 CreatePerspective(float width,
	                                 float height,
	                                 float nearPlane,
	                                 float farPlane) noexcept;
	...
	static Matrix4 CreateOrthographic(float width,
	                                  float height,
	                                  float zNearPlane,
	...
	// static Matrix4 CreateLookAt(Vector3D eye,
	//                             Vector3D focus,
	//                             Vector3D up);
};
