...
namespace Engine
{
	CameraRenderComponent::CameraRenderComponent(const EntityID& ownerID, 
		...
		m_ConstantBuffer{ nullptr }
	{
		...
		{
			Debug::Log("Attempting to Attach a CameraRenderComponent to a game camera with null RenderData!");
			return;
		}
	...
	void CameraRenderComponent::SetTexture(SharedPtr<TextureResource> texture)
	{
		m_TextureResource = texture;
	}

	void CameraRenderComponent::RemoveTexture()
	{
		m_TextureResource = nullptr;
	}

	void CameraRenderComponent::Draw(CameraComponent& camera)
	{
	...
	void CameraRenderComponent::Draw(EditorCameraComponent& camera)
	{
		RenderObjectData* constant = new RenderObjectData();
		constant->Model = m_Transform->GetLocalMatrix();
		constant->ViewProjection = camera.GetViewProjMatrix();
